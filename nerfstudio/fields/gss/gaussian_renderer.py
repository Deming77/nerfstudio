...
from nerfstudio.fields.gss.utils.sh_utils import eval_sh
...
def render(
    viewpoint_camera: GssCamera,
    pc: GaussianModel,
    pipe: RenderConfig,
    bg_color: torch.Tensor,
    mip_kernel_size: float,
    mip_subpixel_offset: torch.Tensor,
    ...
    if mip_kernel_size <= 0:
        raster_settings = original_rasterizer.GaussianRasterizationSettings(
        ...
        rasterizer = original_rasterizer.GaussianRasterizer(raster_settings=raster_settings)
    else:
        if mip_subpixel_offset is None:
            mip_subpixel_offset = torch.zeros((int(viewpoint_camera.image_height), int(viewpoint_camera.image_width), 2), dtype=torch.float32, device="cuda")

        raster_settings = mip_rasterizer.GaussianRasterizationSettings(
            image_height=int(viewpoint_camera.image_height),
            image_width=int(viewpoint_camera.image_width),
            tanfovx=tanfovx,
            tanfovy=tanfovy,
        ...
        rasterizer = mip_rasterizer.GaussianRasterizer(raster_settings=raster_settings)
