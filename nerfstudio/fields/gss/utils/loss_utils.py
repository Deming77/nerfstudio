...
def _ssim(img1:Tensor, img2, window, window_size, channel, size_average=True, mask: Tensor|None=None):
    ...
    if size_average:
        if mask is not None:
            kernel = torch.ones(window_size, window_size, device=mask.device)
            eroded_mask = 1 - torch.clamp(
            ...
            eroded_mask = eroded_mask[None].expand((3, -1, -1))
            return ssim_map[eroded_mask].mean()
